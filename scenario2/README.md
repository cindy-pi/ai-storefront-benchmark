# Scenario 2 — Full Project Brief (Paul's Run, AI-Judged)
...
Each model runs Scenario 2 in a Digiswarm session on its own dedicated repo, using the standard benchmark configuration.
...
Run the pre-flight script to verify token scopes, repo access, Pages configuration, and that the repo is a clean slate:
...
Scenario 2 is scored by AI in a blind peer review. Each of the four models independently reviews all four submissions without knowing which model built which site.
