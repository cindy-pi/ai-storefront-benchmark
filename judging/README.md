...
Once all four models have completed the two scenarios, the results are scored by AI in a blind peer review. A separate Digiswarm project called **judges** is used for this — it has four agents, each running a different model, each acting as an independent judge.
...
6. Do **not** use the default `digiswarm` project — create a dedicated `judges` project so model assignments are persistent across both judging sessions
...
For **Scenario 1**, each judge gives a single overall score per submission plus written notes.

For **Scenario 2**, each judge scores three dimensions per submission:
...
The final site score for Scenario 2 is the average of all four judges' averaged dimension scores. Full methodology is in [scenario2/SCORING.md](../scenario2/SCORING.md).

---

## The Judging Prompts

Two prompts are delivered to Paul in sequence — one per scenario. Deliver each prompt only after the previous scenario's judging session is complete.
...
### Judging Prompt 2 — Scenario 2 (Full Project)

> All four models have completed Scenario 2. Each built a complete static e-commerce site called Fizban's Wands and deployed it to GitHub Pages.
>
> Your task is to coordinate a blind peer review. Use the same Submission A–D mapping from Scenario 1 so the blind is consistent across both judging sessions.
>
> The live site URLs for each submission are listed in [scenario2/README.md](../scenario2/README.md).
>
...
> - **Elegance** — Is the UI polished, cohesive, and pleasant to use?
>
> Full scoring rubric: [scenario2/SCORING.md](../scenario2/SCORING.md)
>
...
**Scenario 1:** The four judges' scores for each submission are averaged into a single final score per submission.
